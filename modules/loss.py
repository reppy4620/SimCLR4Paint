...
class NT_Xent(nn.Module):
    ...
    def get_mask(batch_size):
        # this method makes mask sized (2*batch_size x 2*batch_size)
        m = torch.ones(batch_size, batch_size).fill_diagonal_(0).bool()
        m2 = torch.cat([m, m], dim=0)
        mask = torch.cat([m2, m2], dim=1)
        return mask

    # If you couldn't understand behavior, try running in console one line by one line with the use of small batch size
    # and check tensor just allocated.
    def forward(self, z_i, z_j):
        z = torch.cat((z_i, z_j), dim=0)  # 2B x C
        # calculate similarity
        # As z_i and z_j are normalized in ResNet module, only calculate dot product.
        ...
        neg = sim[self.mask].view(self.batch_size * 2, -1)  # 2B x (2B - 1)
